...
TEXT_INPUT_PATH = 'Data/PronunciationEmbeds/'
ACOUSTIC_INPUT_PATH = 'Data/AcousticFeatures/'
VISUAL_INPUT_PATH = 'Data/VideoFeatures/'

MODEL_OUTPUT_DIR = 'models/MOSES/'
RESULT_OUTPUT_DIR = 'results/MOSES/'
...
class ModalityFusion(nn.Module):
    def __init__(self, dim_model: int):
        ...
        self.TT = MAF_TT(dim_model)
        self.Ta = MAF_Ta(dim_model)
        self.tA = MAF_tA(dim_model)
        self.Tv = MAF_Tv(dim_model)
        self.tV = MAF_tV(dim_model)
        self.Tav = MAF_Tav(dim_model)
        self.tAv = MAF_tAv(dim_model)
        self.taV = MAF_taV(dim_model)
        
        self.final_layer_norm = nn.LayerNorm(dim_model)
        
    def forward(self, text_input: torch.Tensor, acoustic_context: Optional[torch.Tensor] = None, visual_context: Optional[torch.Tensor] = None, tts_context: Optional[torch.Tensor] = None):
        TT = self.TT(text_input, tts_context)
        
        Ta = self.Ta(TT, acoustic_context)
        tA = self.tA(TT, acoustic_context)
        
        Tv = self.Tv(TT, visual_context)
        tV = self.tV(TT, visual_context)
        
        Tav = self.Tav(TT, acoustic_context, visual_context)
        tAv = self.tAv(TT, acoustic_context, visual_context)
        taV = self.taV(TT, acoustic_context, visual_context)
        ...
        wt_Tav = F.sigmoid(Tav)
        wt_tAv = F.sigmoid(tAv)
        wt_taV = F.sigmoid(taV)
        
        output = self.final_layer_norm(text_input + wt_TT * TT + wt_Ta * Ta + wt_tA * tA + wt_Tv * Tv + wt_tV * tV + wt_Tav * Tav + wt_tAv * tAv + wt_taV * taV)
...
class MultimodalBartEncoder(BartPretrainedModel):
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.tts_transformer = TransformerEncoder(d_model=TTS_DIM, 
                                                       num_layers=4,
                                                       num_heads=2, 
                                                       dim_feedforward=TTS_DIM)
        self.MAF_layer = ModalityFusion(dim_model=embed_dim)
        # =============================================================================== #
    ...
    def forward(
        ...
        tts_input=None,      # New addition of tts_input
        head_mask=None,
        inputs_embeds=None,
    ...
    def forward(
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(
                ...
                tts_input=tts_input,      # New addition of tts_input
                head_mask=head_mask,
...
def prepare_dataset(text_path: str,
    ...
    df['visual_features'] = visual_data['video_feats']
    
    # Reading TTS Data
    df['tts_features'] =  [d['TTS_feats'] for d in data.values()]
...
def preprocess_dataset(dataset,
    ...
    model_inputs['tts_input'] = torch.stack([pad_seq(torch.tensor(tf, dtype=torch.float),
                                                        dim=TTS_DIM,
                                                        max_len=TTS_MAX_LEN)
                                                for tf in dataset['tts_features'].values.tolist()], 0).to(DEVICE)
...
def train_epoch(model,
    ...
    for step, batch in enumerate(tqdm(data_loader, desc="Training Iteration")):
        ...
        input_ids, attention_mask, acoustic_input, visual_input, tts_input, labels = batch
        optimizer.zero_grad()
...
def train(model,
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        path = MODEL_OUTPUT_DIR + "MOSES_epoch_"  + TARGET_COLUMN + "_epoch_" + str(epoch+1) + "_" + datetime.now().strftime('%d-%m-%Y-%H:%M')
        print(path)
        save_model(model,
                   path,
                   tokenizer)
...
#             print(name)
    ...
    train_dataset = set_up_data_loader(text_path = TEXT_INPUT_PATH + 'train_data_final_plain_tts.json',
    ...
    val_dataset = set_up_data_loader(text_path = TEXT_INPUT_PATH + 'val_data_final_plain_tts.json',
    ...
    test_dataset = set_up_data_loader(text_path = TEXT_INPUT_PATH + 'test_data_final_plain_tts.json',
